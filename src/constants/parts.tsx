 export const part=[
        {
            name:"Engine Parts",
            desc:"Pistons, cylinders, crankshafts, gaskets and other engine components.",
            icon:(
              <svg
        className="w-full h-full drop-shadow-md"
        viewBox="0 0 512 512"
        xmlns="http://www.w3.org/2000/svg"
        xmlSpace="preserve"
      >
        <defs>
          {/* The Metallic Purple Gradient */}
          <linearGradient id="purpleMetal" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#2E1A47" />    {/* Deep Shadow */}
            <stop offset="20%" stopColor="#4D2F8C" />   {/* Base Color */}
            <stop offset="45%" stopColor="#B39DDB" />   {/* Bright Chrome Sheen */}
            <stop offset="55%" stopColor="#D1C4E9" />   {/* High Polish Reflect */}
            <stop offset="80%" stopColor="#4D2F8C" />   {/* Return to Base */}
            <stop offset="100%" stopColor="#211035" />  {/* Dark Edge */}
          </linearGradient>

          {/* Optional: Subtle Glow Filter */}
          <filter id="metalGlow" x="-20%" y="-20%" width="140%" height="140%">
            <feGaussianBlur stdDeviation="1" result="blur" />
            <feComposite in="SourceGraphic" in2="blur" operator="over" />
          </filter>
        </defs>

        <g fill="url(#purpleMetal)" style={{ filter: 'url(#metalGlow)' }}>
          {/* Main Body */}
          <polygon points="351.067,170.863 322.93,145.913 276.4,145.913 276.4,123.219 291.366,123.219 291.366,89.828 172.742,89.828 
            172.742,123.219 187.709,123.219 187.709,145.913 139.265,145.913 111.127,170.863 66.783,170.863 66.783,187.559 66.783,253.795 
            33.391,253.795 33.391,211.179 0,211.179 0,329.804 33.391,329.804 33.391,287.186 66.783,287.186 66.783,353.422 66.783,370.118 
            139.265,370.118 197.974,422.172 395.388,422.172 395.388,170.863"/>
          
          {/* Side Plate */}
          <path d="M494.32,196.801l-4.858-8.131h-60.683v214.625h60.683l4.858-8.132c1.81-3.027,17.68-31.537,17.68-99.181
            S496.13,199.829,494.32,196.801z"/>
        </g>
      </svg>
            )
        },
        {
            name:"Brake Parts",
            desc:"Brake pads, brake discs, brake pumps and braking system components.",
            icon:(
               <svg
        className="w-full h-full drop-shadow-md"
        viewBox="-4.57 0 101.977 101.977"
        xmlns="http://www.w3.org/2000/svg"
      >
        <defs>
          {/* Reusing the Purple Metal Gradient */}
          <linearGradient id="purpleMetalBrake" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#2E1A47" />    {/* Deep Shadow */}
            <stop offset="20%" stopColor="#4D2F8C" />   {/* Base Color */}
            <stop offset="45%" stopColor="#B39DDB" />   {/* Bright Chrome Sheen */}
            <stop offset="55%" stopColor="#D1C4E9" />   {/* High Polish Reflect */}
            <stop offset="80%" stopColor="#4D2F8C" />   {/* Return to Base */}
            <stop offset="100%" stopColor="#211035" />  {/* Dark Edge */}
          </linearGradient>

          {/* Glow Filter for that premium finish */}
          <filter id="brakeGlow" x="-20%" y="-20%" width="140%" height="140%">
            <feGaussianBlur stdDeviation="0.8" result="blur" />
            <feComposite in="SourceGraphic" in2="blur" operator="over" />
          </filter>
        </defs>

        <g fill="url(#purpleMetalBrake)" style={{ filter: 'url(#brakeGlow)' }}>
          <path 
            id="brake_disks" 
            data-name="brake disks" 
            d="M467.554,786.732a5.09,5.09,0,1,0,5.087,5.089A5.09,5.09,0,0,0,467.554,786.732Zm0,6.179a1.089,1.089,0,1,1,1.089-1.09A1.095,1.095,0,0,1,467.554,792.911Zm24.558-33.287a5.09,5.09,0,1,0,5.09,5.092A5.09,5.09,0,0,0,492.112,759.624Zm0,6.18a1.09,1.09,0,1,1,1.089-1.088A1.095,1.095,0,0,1,492.112,765.8Zm-24.558,4a5.09,5.09,0,1,0-5.09-5.088A5.09,5.09,0,0,0,467.554,769.8Zm0-6.179a1.091,1.091,0,0,1,1.089,1.091h0a1.089,1.089,0,1,1-1.087-1.091Zm24.558,23.107a5.09,5.09,0,1,0,5.09,5.089A5.092,5.092,0,0,0,492.112,786.732Zm0,6.179a1.089,1.089,0,1,1,1.089-1.09A1.094,1.094,0,0,1,492.112,792.911Zm-.034-14.644a12.245,12.245,0,1,0-12.244,12.244A12.245,12.245,0,0,0,492.078,778.267Zm-12.244,8.245a8.244,8.244,0,1,1,8.243-8.245A8.257,8.257,0,0,1,479.834,786.512Zm29.659-43.958L506.985,730.6a2,2,0,0,0-1.014-1.354,55.56,55.56,0,0,0-50.712-.8,1.993,1.993,0,0,0-1.071,1.383l-2.4,11.452a46.418,46.418,0,1,0,57.708,1.272ZM457.9,731.615a51.582,51.582,0,0,1,45.364.726l1.406,6.706a46.213,46.213,0,0,0-8.505-4.251,2,2,0,1,0-1.408,3.745h0a42.338,42.338,0,0,1,9.7,5.174l-3.786,3.327a37.52,37.52,0,0,0-39.861-1.128l-3.839-3.377a42.318,42.318,0,0,1,9.156-4.428,2,2,0,1,0-1.293-3.784,46.193,46.193,0,0,0-8.29,3.783Zm51.931,76.65a42.533,42.533,0,0,1-11.669,8.272l6.342-15.563a2,2,0,1,0-3.7-1.51l-7.792,19.123,0,.01a42.441,42.441,0,0,1-43.168-10.332q-1.494-1.494-2.834-3.133l14.089,2.252a2.044,2.044,0,0,0,.32.025,2,2,0,0,0,.312-3.974l-17.58-2.813a2.008,2.008,0,0,0-.378-.021,42.444,42.444,0,0,1-3.891-36.6l6.2,15.074a2,2,0,1,0,3.7-1.521l-7.681-18.682a42.63,42.63,0,0,1,11.524-13.951l5.657,4.975a2,2,0,0,0,2.407.176,33.518,33.518,0,0,1,37.944,1.124,2.005,2.005,0,0,0,2.5-.112l5.514-4.848a42.363,42.363,0,0,1,14,24.842l-11.748-9.641a2,2,0,1,0-2.537,3.091l14.868,12.2c.018.51.029,1.022.029,1.537A42.286,42.286,0,0,1,509.831,808.265Zm-29.995-58.653a28.655,28.655,0,1,0,28.651,28.655A28.659,28.659,0,0,0,479.836,749.612Zm0,53.308a24.654,24.654,0,1,1,24.651-24.653A24.688,24.688,0,0,1,479.836,802.92Z" 
            transform="translate(-433.409 -722.714)"
          />
        </g>
      </svg>
            )
        },
        {
            name:"Body Panels",
            desc:"Fairings, mudguards, side panels and exterior bike components.",
            icon:(
              <svg
        className="w-full h-full drop-shadow-md"
        viewBox="0 0 578.924 578.924"
        xmlns="http://www.w3.org/2000/svg"
        xmlSpace="preserve"
      >
        <defs>
          {/* Consistent Metallic Purple Gradient */}
          <linearGradient id="purpleMetalEngine" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#2E1A47" />    {/* Deep Shadow */}
            <stop offset="20%" stopColor="#4D2F8C" />   {/* Base Color */}
            <stop offset="45%" stopColor="#B39DDB" />   {/* Bright Chrome Sheen */}
            <stop offset="55%" stopColor="#D1C4E9" />   {/* High Polish Reflect */}
            <stop offset="80%" stopColor="#4D2F8C" />   {/* Return to Base */}
            <stop offset="100%" stopColor="#211035" />  {/* Dark Edge */}
          </linearGradient>

          {/* Glow Filter */}
          <filter id="engineGlow" x="-20%" y="-20%" width="140%" height="140%">
            <feGaussianBlur stdDeviation="1.2" result="blur" />
            <feComposite in="SourceGraphic" in2="blur" operator="over" />
          </filter>
        </defs>

        <g fill="url(#purpleMetalEngine)" style={{ filter: 'url(#engineGlow)' }}>
          <path d="M198.689,290.949c-1.167,0-2.553-0.344-4.045-0.976l-88.864-37.915c-4.6-1.951-8.664-2.955-12.125-2.955
          c-5.852,0-8.645,2.917-9.792,4.657c-0.679,1.033-1.415,2.831-1.492,5.202C35.964,265.035,0,304.728,0,352.751
          c0,52.221,42.477,94.688,94.688,94.688c37.399,0,69.71-21.859,85.077-53.416c5.757,2.524,11.8,4.064,17.337,4.064H365.02
          c12.346,0,23.678-8.157,28.229-19.402c11.312,39.627,47.773,68.754,90.986,68.754c52.213,0,94.689-42.477,94.689-94.688
          c0-39.254-24.031-73.01-58.141-87.334c2.361-4.036,3.758-8.367,3.758-12.575c0-10.155-5.729-26.077-13.033-36.232l-36.902-51.245
          c-7.381-10.261-22.738-20.866-34.951-24.136l-29.92-8.023c-4.189-1.128-8.568-1.721-12.633-1.721
          c-9.4,0-16.074,2.983-18.771,8.376c-4.58,9.17,4.398,20.779,15.635,27.521l15.816,9.496c3.156,1.893,4.285,3.863,4.303,5.078
          c0.01,1.233-1.117,2.668-3.107,3.93c-2.438,1.54-4.465,2.419-5.977,2.831c-2.104-8.979-13.99-14.765-25.742-15.979l-106-10.968
          c-13.703-1.435-29.816,6.837-36.72,18.599l-8.147,13.923c-4.399,7.488-15.874,14.057-24.547,14.057h-26.88
          c-8.558,0-19.498-6.665-23.438-14.268l-6.665-12.89c-6.245-12.078-21.774-21.535-35.372-21.535H39.273
          c-3.366,0-13.636,0-15.529,7.832c-1.894,7.832,7.239,12.518,10.232,14.066l152.216,78.145c7.774,3.997,15.233,15.147,15.94,23.858
          l0.077,0.976c0.201,2.524-0.201,4.562-1.147,5.575C200.87,290.308,200.287,290.949,198.689,290.949z"/>
        </g>
      </svg>
            )
        },
        {
            name:"Electrical Parts",
            desc:"CDI units, wiring harnesses, sensors, switches and lighting components.",
            icon:(
               <svg
        className="w-full h-full drop-shadow-md"
        viewBox="0 0 503.607 503.607"
        xmlns="http://www.w3.org/2000/svg"
        xmlSpace="preserve"
      >
        <defs>
          {/* Unique ID for the Energy Icon Gradient */}
          <linearGradient id="purpleMetalEnergy" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#2E1A47" />    {/* Deep Shadow */}
            <stop offset="20%" stopColor="#4D2F8C" />   {/* Base Color */}
            <stop offset="45%" stopColor="#B39DDB" />   {/* Bright Chrome Sheen */}
            <stop offset="55%" stopColor="#D1C4E9" />   {/* High Polish Reflect */}
            <stop offset="80%" stopColor="#4D2F8C" />   {/* Return to Base */}
            <stop offset="100%" stopColor="#211035" />  {/* Dark Edge */}
          </linearGradient>

          {/* Consistent Glow Filter */}
          <filter id="energyBoltGlow" x="-20%" y="-20%" width="140%" height="140%">
            <feGaussianBlur stdDeviation="1" result="blur" />
            <feComposite in="SourceGraphic" in2="blur" operator="over" />
          </filter>
        </defs>

        <g fill="url(#purpleMetalEnergy)" style={{ filter: 'url(#energyBoltGlow)' }}>
          <path d="M252.836,213.877c-1.469-2.686-1.368-5.959,0.277-8.553l50.554-79.419h-46.953l-66.098,117.508h41.313
            c2.803,0,5.414,1.402,6.975,3.727c1.553,2.333,1.855,5.288,0.781,7.873l-35.479,85.588l104.893-122.368h-48.9
            C257.134,218.233,254.314,216.554,252.836,213.877z"/>
          <path d="M251.802,67.151c-101.821,0-184.656,82.835-184.656,184.656c0,101.812,82.835,184.656,184.656,184.656
            s184.656-82.843,184.656-184.656C436.457,149.986,353.622,67.151,251.802,67.151z M333.713,215.295L182.631,391.557
            c-1.637,1.922-3.995,2.938-6.379,2.938c-1.494,0-3.022-0.403-4.381-1.242c-3.517-2.165-4.944-6.555-3.366-10.366L219.361,260.2
            h-43.1c-2.98,0-5.733-1.586-7.244-4.163c-1.511-2.568-1.536-5.749-0.067-8.351l75.541-134.295
            c1.486-2.635,4.281-4.272,7.311-4.272h67.148c3.064,0,5.884,1.67,7.361,4.348c1.469,2.686,1.368,5.959-0.277,8.553
            l-50.545,79.427h51.855c3.282,0,6.253,1.905,7.621,4.885C336.332,209.302,335.854,212.811,333.713,215.295z"/>
          <path d="M453.246,0H50.361C22.587,0,0,22.587,0,50.361v402.885c0,27.774,22.587,50.361,50.361,50.361h402.885
            c27.774,0,50.361-22.587,50.361-50.361V50.361C503.607,22.587,481.02,0,453.246,0z M50.361,33.574
            c9.275,0,16.787,7.521,16.787,16.787c0,9.266-7.512,16.787-16.787,16.787s-16.787-7.521-16.787-16.787
            C33.574,41.094,41.086,33.574,50.361,33.574z M50.361,470.033c-9.275,0-16.787-7.521-16.787-16.787s7.512-16.787,16.787-16.787
            s16.787,7.521,16.787,16.787S59.635,470.033,50.361,470.033z M251.803,453.246c-111.07,0-201.443-90.372-201.443-201.443
            S140.733,50.361,251.803,50.361s201.443,90.372,201.443,201.443S362.874,453.246,251.803,453.246z M453.246,470.033
            c-9.275,0-16.787-7.521-16.787-16.787s7.512-16.787,16.787-16.787c9.275,0,16.787,7.521,16.787,16.787
            S462.521,470.033,453.246,470.033z M453.246,67.148c-9.275,0-16.787-7.521-16.787-16.787c0-9.266,7.512-16.787,16.787-16.787
            c9.275,0,16.787,7.521,16.787,16.787C470.033,59.627,462.521,67.148,453.246,67.148z"/>
        </g>
      </svg>
            )
        },
        {
          
            name:"Suspension",
            desc:"Front forks, rear shock absorbers and suspension components.",
            icon:(
               <svg
        className="w-full h-full drop-shadow-md rotate-25"
        viewBox="-6.79 0 20 20"
        xmlns="http://www.w3.org/2000/svg"
      >
        <defs>
          {/* Metallic Purple Gradient */}
          <linearGradient id="purpleMetalSusp" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#2E1A47" />    {/* Deep Shadow */}
            <stop offset="20%" stopColor="#4D2F8C" />   {/* Base Color */}
            <stop offset="45%" stopColor="#B39DDB" />   {/* Bright Chrome Sheen */}
            <stop offset="55%" stopColor="#D1C4E9" />   {/* High Polish Reflect */}
            <stop offset="80%" stopColor="#4D2F8C" />   {/* Return to Base */}
            <stop offset="100%" stopColor="#211035" />  {/* Dark Edge */}
          </linearGradient>

          {/* Glow Filter */}
          <filter id="suspensionGlow" x="-20%" y="-20%" width="140%" height="140%">
            <feGaussianBlur stdDeviation="0.5" result="blur" />
            <feComposite in="SourceGraphic" in2="blur" operator="over" />
          </filter>
        </defs>

        <g transform="translate(-8.787 -2)" style={{ filter: 'url(#suspensionGlow)' }}>
          {/* The top small square - Filled */}
          <rect 
            id="secondary" 
            fill="url(#purpleMetalSusp)" 
            width="2" 
            height="2" 
            transform="translate(13 5) rotate(180)"
          />
          
          {/* The suspension lines and spring - Stroked */}
          <path 
            id="primary" 
            d="M10,19a2,2,0,1,1,2,2A2,2,0,0,1,10,19ZM12,5V17M11,5h2V3H11Zm-1,5.5,4,1m0,3-4-1m4-5-4-1" 
            fill="none" 
            stroke="url(#purpleMetalSusp)" 
            strokeLinecap="round" 
            strokeLinejoin="round" 
            strokeWidth="1.5" 
          />
        </g>
      </svg>
            )
        },
        {
            name:"Used / Rare Parts",
            desc:"Hard-to-find or discontinued parts sourced from dismantlers and garages.",
            icon:(
            <svg
        viewBox="0 0 97.724 97.724"
        xmlns="http://www.w3.org/2000/svg"
        xmlSpace="preserve"
        className="w-full h-full drop-shadow-md"
      >
        <defs>
          {/* Consistent Metallic Purple Gradient */}
          <linearGradient id="purpleMetalExchange" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#2E1A47" />    {/* Deep Shadow */}
            <stop offset="20%" stopColor="#4D2F8C" />   {/* Base Color */}
            <stop offset="45%" stopColor="#B39DDB" />   {/* Bright Chrome Sheen */}
            <stop offset="55%" stopColor="#D1C4E9" />   {/* High Polish Reflect */}
            <stop offset="80%" stopColor="#4D2F8C" />   {/* Return to Base */}
            <stop offset="100%" stopColor="#211035" />  {/* Dark Edge */}
          </linearGradient>

          {/* Glow Filter */}
          <filter id="exchangeGlow" x="-20%" y="-20%" width="140%" height="140%">
            <feGaussianBlur stdDeviation="1" result="blur" />
            <feComposite in="SourceGraphic" in2="blur" operator="over" />
          </filter>
        </defs>

        <g fill="url(#purpleMetalExchange)" style={{ filter: 'url(#exchangeGlow)' }}>
          <g>
            <path
              d="M49.462,12.579V1.48c0-0.544-0.301-1.042-0.795-1.304c-0.488-0.26-1.069-0.231-1.521,0.073L16.483,20.888
                c-0.411,0.282-0.652,0.739-0.652,1.229c0,0.491,0.238,0.957,0.652,1.23L47.132,43.98c0.459,0.305,1.033,0.335,1.528,0.087
                c0.479-0.263,0.788-0.768,0.788-1.314V30.822c4.827,0.034,8.804,0.241,12.137,0.64c15.947,1.899,20.212,11.466,20.387,11.873
                c0.222,0.553,0.776,0.9,1.365,0.9c0.085,0,0.191-0.003,0.287-0.027c0.688-0.137,1.191-0.743,1.191-1.448
                C84.815,18.591,56.116,13.444,49.462,12.579z"
            />
            <path
              d="M81.239,74.371L50.583,53.742c-0.445-0.308-1.026-0.343-1.521-0.093c-0.479,0.268-0.78,0.767-0.78,1.314v11.933
                c-4.833-0.037-8.81-0.243-12.136-0.644c-15.954-1.898-20.22-11.464-20.388-11.871c-0.231-0.555-0.782-0.9-1.364-0.9
                c-0.094,0-0.198,0.004-0.294,0.028c-0.69,0.137-1.191,0.742-1.191,1.45c0,24.164,28.696,29.318,35.358,30.188v11.098
                c0,0.537,0.294,1.041,0.788,1.304c0.488,0.257,1.069,0.229,1.521-0.075l30.663-20.636c0.41-0.284,0.653-0.739,0.653-1.231
                C81.893,75.11,81.653,74.642,81.239,74.371z"
            />
          </g>
        </g>
      </svg>
            )
        },
    ]