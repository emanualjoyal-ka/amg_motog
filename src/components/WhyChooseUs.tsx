...
const WhyChooseUs = () => {
    ...
    {
      title:'Quality Checked Before Listing',
      icon:(
    ...
    <div className="mt-10">
          {
            ChooseUS.map((item,index)=>(
              <div key={index} className='flex items-center gap-2 md:gap-4 mt-5'>
                <div className='w-10 h-10 border border-white/30 md:w-15 md:h-15 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center'>
                
                    {item.icon}
                  
                </div>
                <h3 className='font-bold md:font-medium text-white md:text-xl'>{item.title}</h3>
              </div>
            ))
          }
        </div>
